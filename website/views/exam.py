...
def section(section_id, exam_id):
    if request.method == 'POST':
        ...
        return redirect(url_for('exam.section', section_id=section_id, exam_id=exam_id))
    data = Questions.query.filter_by(section_id=section_id).first().question_page
...
def get_exam_id():
    ...
    ids = [row.exam_id for row in query.all()]
